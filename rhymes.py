...
Also at github: https://github.com/cmusphinx/cmudict (this might be better maintained, 
but the format has changed a bit, so the code below would need to be tweaked)
...
CMU dict uses an enriched form of ARPAbet: https://en.wikipedia.org/wiki/ARPABET

CMU uses the 2-letter variant, with stress indicators. Stress is indicated by digits 
appended to vowels, and is a little confusing: 1 is primary stress,
...
# This is just to get us something to play with, it's not the right way to do it!


def cmu_data_from_nltk():
    entries = nltk.corpus.cmudict.entries() # nltk may require some steps
    # for this to work...
    to_syllables = dict(entries) # oops! Just overwrote any alternate entries, sorry!
...
# download current CMU data from  http://svn.code.sf.net/p/cmusphinx/code/trunk/cmudict/
# note that github version modified format, so can't use that here
# this time, I'm being a little fancier and not clobbering alternates - this makes
# handling the results a little more annoying, though
...
def read_cmu_dict(path_to_cmu_dict):
    ...
    for line in f:
        if line.startswith(";"):
            continue
        # CMU dict format uses 2 spaces to separate an entry from its syllabalization
...
# It may turn out that we prefer to write this stuff to a sqlite db and read it from
# disk rather than keep it in memory. This would have some advantages, including
# hitting one of the items on the CMU dict maintainers' wish list.
...
def get_stress(phoneme):
    '''
    From a phoneme representation, return the stress marker (an integer in [0, 1, 2])
    :return: stress marker or None if no marker exists
    '''
    if len(phoneme) == 0: return
    end = phoneme[-1]
    return int(end) if end.isnumeric() else None

def humanize_stress(stress):
    '''
    Takes a CMU-defined stress (1 is primary stress, 2 is secondary stress, 0 is no stress) and
    returns an integer such that primary stress > secondary stress > no stress.
...
def split_on_stress(phonemes):
    '''
    Takes a list of phonemes and returns the list as two elements: the first is the portion of the word
    up to but not including the max stressed vowel, and the second the portion after, including the vowel.
    :param phonemes:
...
def split_on_final_vowel(phonemes):
    '''
    Given a list of phonemes splits the list in two where the first list contains the phonemes up to
    (but not including) the final vowel, and the second list is the final vowel and everything rightwards.
    ...
    try:
        i_stress = next(i for i in reversed(range(len(phonemes))) if stresses[i] is not None)
...
def last_word(line, delimiter=" "):
    '''
    Given a string, returns the part to the right of the last instance of the given delimiter.
    ...
    try:
        ...
        print("Did not find whitespace in string \"{}\". Returning input.".format(clean_line))
        return line
    return line[index + 1:]

def eval_periphery(l1, l2, current_pts=0, possible_pts=0, unit_pts=1, nucleus_pts=0):
    '''
    Takes two arrays of phonemes and compares them in parallel. Returns updated current_points
    and possible_points

    :param l1: list of phonemes
    :param l2: list of phonemes
    :param current_pts: aggregator for if you're comparing parts of words independently
    :param possible_pts: aggregator for if you're comparing parts of words independently
    :param unit_pts: number of points to assign a matching pair of phonemes
    :param nucleus_pts: number of points to assign the first phoneme pair in the list if they  match
...
def rate_rhyme(word_1, word_2):
    """
    Takes two words (in English...might want to adapt this to also accept phoneme lists) and returns
    an estimate of how closely they rhyme. Values are between [0, 1], where higher values mean a closer match.

    This evaluation is really rough right now. I think the next place I want to go with it is to take
    phoneme classes into account so that "bat" and "cat" returns a higher value than "bat" and "bath"
    ...
    # - for each position past the nucleus, give +5 points for each perfectly matching phone
    # rate "onset" as follows:
    # - for each position prior to the nucleus, give +1 point for each perfectly matching phone
    # syllables too?

    To be honest, you'd get pretty much the same result if you only looked at the rhyme nucleus, so the bit
    ...
    current_points, possible_points = eval_periphery(coda_1, coda_2,
                                                     ...
                                                     nucleus_pts=NUCLEUS, unit_pts=CODA)
    current_points, possible_points = eval_periphery(onset_1, onset_2,
                                                     current_points, possible_points,
                                                     unit_pts=ONSET)
...
stripped = list(filter(lambda pot_line: len(pot_line) > 0, shropshire1))  # empty lines are a crutch!
...
'''
  ...
  - if it rhymes with a known group, add the pair there. if not, make a new rhyme group
'''
